...
- **主会话思考强度**:在输入区工具栏的 `conversation.input.left` 座位(模型选择器旁)放一条滑条。档位取自当前模型目录里该模型自己声明的 `reasoning.efforts`,取值即该会话的持久选择,改动经由官方每会话模型目录(`ctx.modelDirectories`)提交——与内置 `/model` 弹层写的是同一份状态,两处不会各说各话。目录里未声明任何档位的模型不渲染滑条。
