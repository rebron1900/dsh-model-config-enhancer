...
- **Composer thinking strength**: seats a range slider at `conversation.input.left`, in the composer tool row beside the model selector. Its steps are the current model's own catalog `reasoning.efforts`, its value is the session's durable selection, and a change commits through the official per-session model directory (`ctx.modelDirectories`) — the same shared state the built-in `/model` popup writes, so the two surfaces never disagree. A model whose catalog declares no efforts renders no slider at all.
